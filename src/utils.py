...
def fixed_sincos2d_embeddings(ncols: int, nrows: int, dim: int) -> Array:
    """ https://github.com/huggingface/pytorch-image-models/blob/6b9e38b5891e1abbf63827239113ec30cbe631aa/timm/layers/pos_embed_sincos.py#L39-L77 """
...
def fixed_sincos1d_embeddings(max_len: int, dim: int) -> jnp.ndarray:
    freqs = 1 / (10000 ** jnp.linspace(0, 1, dim // 2))
    pos = jnp.arange(0, max_len, dtype=jnp.float32)[:, None]
    pos_enc = jnp.dot(pos, freqs[None, :])

    sin_enc = jnp.sin(pos_enc)
    cos_enc = jnp.cos(pos_enc)
    embeddings = jnp.concatenate([sin_enc, cos_enc], axis=1)
    return embeddings
...
def load_pretrained_params(args: argparse.Namespace, params: ArrayTree, finetune:bool) -> ArrayTree:
    with wds.gopen(args.pretrained_ckpt) as fp:
        new_params = flax.serialization.msgpack_restore(fp.read())

    # Remove decoder related parameters if finetuning only the encoder.
    if finetune:
        new_params = {k: v for k, v in new_params.items() if not k.startswith("decoder")}
...
def load_pretrained_mlm_embedding(args: argparse.Namespace, params: ArrayTree):
    """ References
    - Finetuning with bert-base-cased: https://colab.research.google.com/github/huggingface/notebooks/blob/master/examples/text_classification_flax.ipynb#scrollTo=TlqNaB8jIrJW
    - Using GPT pretrained model with cross attention: https://github.com/snoop2head/MobileVSR/blob/cmts%2Blm/finetuning/src/training.py
    - Flax documentation with FlaxClip Model: https://flax.readthedocs.io/en/latest/guides/training_techniques/transfer_learning.html
    - FlaxBertModel Source: https://github.com/huggingface/transformers/blob/main/src/transformers/models/bert/modeling_flax_bert.py
    """

    bert_module = FlaxBertModel.from_pretrained(args.pretrained_mlm, from_pt=True)
    pretrained_embedding = bert_module.params["embeddings"]["word_embeddings"]["embedding"]
    print(f"loading embedding from {args.pretrained_mlm} with shape of {pretrained_embedding.shape}") 
    params["model"]["text_embed"]["wte"]["embedding"] = pretrained_embedding
    del bert_module
    return params